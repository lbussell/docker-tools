...
namespace Microsoft.DotNet.ImageBuilder
{
    public static class ExecuteHelper
    {
            ...
            string? executeMessageOverride = null)
        {
            ...
            executeMessageOverride ??= $"{processInfo.FileName} {processInfo.Arguments}";
            ...
            stopwatch.Start();
            ProcessResult processResult = executor(processInfo);
            stopwatch.Stop();
            s_loggerService.WriteSubheading($"EXECUTION ELAPSED TIME: {stopwatch.Elapsed}");

            if (processResult.Process.ExitCode != 0)
            {
                string exceptionMsg = errorMessage ?? $"Failed to execute {processInfo.FileName} {processInfo.Arguments}";

...
public class ImageDigestCache(IImageDigestService imageDigestService)
{
    private readonly IImageDigestService _inner = imageDigestService;
    ...
    public Task<string?> GetImageDigestAsync(string tag, bool isDryRun) =>
        LockHelper.DoubleCheckedLockLookupAsync(_digestCacheLock, _digestCache, tag,
            () => _inner.GetImageDigestAsync(tag, isDryRun),

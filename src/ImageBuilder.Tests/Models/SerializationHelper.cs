...
public static class SerializationHelper
{
    ...
    /// <summary>
    /// Asserts that deserializing the given JSON throws a <see cref="JsonException"/>
    /// because a required property is missing.
    /// </summary>
    ...
    public static void AssertDeserializationFails<T>(string json, string propertyName)
    {
        JsonException exception = Assert.Throws<JsonException>(
            () => JsonSerializer.Deserialize<T>(json, JsonOptions.DeserializerOptions));

        // STJ error messages vary, but should mention the property name in some form
        exception.Message.ShouldContain(propertyName, Case.Insensitive);
    }
    ...
    /// <summary>
    /// Serializes an object to JSON using the shared JsonOptions.
    /// </summary>
    /// <typeparam name="T">The type of object to serialize.</typeparam>
    /// <param name="obj">The object to serialize.</param>
    ...
    {
        return JsonOptions.Serialize(obj);
    }

    /// <summary>
    /// Deserializes a JSON string to an object using the shared JsonOptions.
    /// </summary>
    /// <typeparam name="T">The type of object to deserialize to.</typeparam>
    /// <param name="json">The JSON string to deserialize.</param>
    ...
    {
        return JsonOptions.DeserializeOrThrow<T>(json);
    }
}

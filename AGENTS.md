...
- **ImageBuilder self-references its own tag.** The embedded `templates/variables/docker-images.yml` is the only Cottle-templated infrastructure file: it stores the ImageBuilder image tag as `{{IMAGE_BUILDER_TAG}}` instead of a concrete tag (a build can't know its own future tag). The `update` command renders only that file and substitutes this build's tag from `AssemblyMetadata("UniqueId")`. `src/manifest.json` passes an `IMAGEBUILDER_TAG` build arg (`$(UniqueId)`) to both Dockerfiles, which expose it as an MSBuild property during `dotnet publish`; the ImageBuilder project emits that value as assembly metadata. When the tag is not baked in (for example, the dotnet tool or a local build), `update` falls back to the `latest` tag and logs a warning. Because of this, the embedded `docker-images.yml` (template) intentionally differs from the rendered `eng/docker-tools/docker-images.yml` (concrete) on that one line. (Digest references — issue #2132 — are out of scope: a build cannot self-know its own digest.)
- **The two copies must be kept in sync.** When changing pipeline templates or scripts, edit `eng/docker-tools/` and mirror the change into `src/Infrastructure/content/` (or edit `src/Infrastructure/content/` and run `update` to regenerate `eng/docker-tools/`). There is no automated test enforcing this. (The `docker-images.yml` self-reference line is the deliberate exception described above.)
